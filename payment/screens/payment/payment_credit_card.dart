...
class _PaymentCreditCardState extends State<PaymentCreditCard> {
  ...
  Widget build(BuildContext context) {
    return OverlayLoadingCustom(loadingWidget: BlocBuilder<AuthBloc, AuthState>(
      builder: (context, state) {
        return LoadingCustom(
            isOverlay: true, isLoading: _isLoading || state.addCardLoading!);
      },
      ...
      onSubmit: (CreditCardModel card) async {
        if (card.cardNumber != '') {
          ...
          String tokenId =
              await MyPluginPayment.createTokenWidthCreditCard(data: card);
          setState(() {
            _isLoading = false;
          });
